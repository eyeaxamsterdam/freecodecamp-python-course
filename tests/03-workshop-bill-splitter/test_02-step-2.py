...
_step = os.path.join(os.path.dirname(__file__), '..', '..', 'steps', '03-workshop-bill-splitter', '02-step-2.py')
...
def test_hint_1():
    ...
    assert _Node(_code).has_variable('num_of_friends')

def test_hint_2():
    """You should assign the integer 4 to your num_of_friends variable."""
    assert _Node(_code).find_variable('num_of_friends').is_equivalent('num_of_friends = 4')
