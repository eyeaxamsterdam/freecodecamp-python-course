...
_step = os.path.join(os.path.dirname(__file__), '..', '..', 'steps', '01-workshop-report-card-printer', '04-step-4.py')
...
def test_hint_1():
    ...
    assert _Node(_code).has_variable("is_student")

def test_hint_2():
    """The variable is_student should store the value True. Do not surround the value with quotes."""
    assert _Node(_code).find_variable("is_student").is_equivalent("is_student = True")
