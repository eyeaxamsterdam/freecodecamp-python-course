...
def test_hint_2():
    ...
    assert _Node(_code).find_variable("employee_code").is_equivalent("employee_code = 'DEV-2026-JD-001'")

def test_hint_3():
    """You should have a variable named department."""
    assert _Node(_code).has_variable("department")

def test_hint_4():
    """You should slice the first three characters of employee_code and assign them to your department variable."""
    assert dep = _Node(_code).find_variable("department")
...
def test_hint_5():
    ...
    assert _Node(_code).has_call("print(department)")
